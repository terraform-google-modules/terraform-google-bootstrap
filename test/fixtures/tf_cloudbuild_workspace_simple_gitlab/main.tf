...
resource "google_secret_manager_secret" "gitlab_api_token" {
  ...
  secret_id = "cb-gl-api-credential-${random_id.suffix.dec}"
...
resource "google_secret_manager_secret" "gitlab_read_api_token" {
  project   = var.project_id
  secret_id = "cb-gl-read-api-credential-${random_id.suffix.dec}"
  replication {
    auto {}
  }
}
...
resource "google_secret_manager_secret" "gitlab_webhook" {
  project   = var.project_id
  secret_id = "cb-gl-webhook-${random_id.suffix.dec}"
  replication {
    auto {}
  }
}
